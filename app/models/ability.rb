class Ability
  ...
  def initialize(user = nil)
    ...
    if user.sgsadmin?
        ...
        can [:read, :discard_index, :print, :rent_report, :rent_report_export, :to_scan, :select_catalog2, :select_catalog3, :select_catalog4, :reprint_import], RentInfo
        # can [:new, :read], Purchase
        can :manage, Sequence
        can :manage, FixedAssetInventory
    ...
    if user.deviceadmin?
        ...
        can :manage, RentInfo
        if (user.unit.unit_level == 3 && !user.unit.is_facility_management_unit) || (user.unit.unit_level == 4)
            cannot [:rent_info_import, :batch_destroy, :discard, :rent_report, :rent_report_export], RentInfo
        end
        can :manage, Purchase
    ...
    if user.accountant?
        ...
        can [:to_do_index, :doing_index, :done_index, :read, :approve, :decline, :print_certificate, :rent_print_certificate], Purchase
