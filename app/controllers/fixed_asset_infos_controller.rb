class FixedAssetInfosController < ApplicationController
        ...
        ActiveRecord::Base.transaction do
          begin
            ...
            4.upto(instance.last_row) do |line|
              ...
              use_at = rowarr[6].blank? ? nil : DateTime.parse(rowarr[7].to_s).strftime('%Y-%m-%d')
              ...
              if !ori_info.blank?
                ori_info.update!(relevant_department: relevant_department)
                ori_infos.delete(asset_no)
              else
                FixedAssetInfo.create!(sn: sn, asset_name: asset_name, asset_no: asset_no, fixed_asset_catalog_id: catalog.id, relevant_department: relevant_department, buy_at:buy_at ,use_at:use_at, measurement_unit:measurement_unit, amount:amount, sum:sum, unit_id: unit.id, branch:branch, location:location, user:user, change_log:change_log, status:"in_use", print_times:0)
              end
            end
    ...
    if !params[:grid].blank?
      if !params[:grid][:f].blank?
        ...
        if !params_f["fixed_asset_catalogs.code"].blank?
          selectinfos=selectinfos.joins(:fixed_asset_catalog).where("fixed_asset_catalogs.code = ?", params_f["fixed_asset_catalogs.code"])
        end
        if !params_f[:relevant_department].blank?
          selectinfos=selectinfos.where(relevant_department: params_f[:relevant_department])
        end
  ...
  def exportinfos_xls_content_for(objs)  
    ...
    objs.each do |obj|  
      ...
      sheet1[count_row,6]=obj.buy_at.strftime('%Y-%m-%d')
      sheet1[count_row,7]=obj.use_at.strftime('%Y-%m-%d')
